...
AverageImageFilter::AverageImageFilter(int factor)
{
	_factor = factor;
}

AverageImageFilter::~AverageImageFilter()
{
}

void AverageImageFilter::SetFactor(int factor)
{
	_factor = factor;
}

int AverageImageFilter::GetFactor()
{
	return _factor;
}

bool AverageImageFilter::Transform(uint8_t *data, int width, int height)
{
	if (IsEnabled() == false) {
		return false;
	}

...
struct lwuit_size_t GridLayout::GetPreferredLayoutSize(Component *parent)
{
	// WARN:: sync parent
	...
	for (int i = 0 ; i < ncomponents ; i++) {
		...
		struct lwuit_size_t d = comp->GetPreferredSize();
	...
	struct lwuit_size_t t = {insets.left + insets.right + ncols*w + (ncols-1)*_hgap, insets.top + insets.bottom + nrows*h + (nrows-1)*_vgap};

	return t;
}

void GridLayout::DoLayout(Component *parent)
{
	...
	int ncomponents = parent->GetComponents().size(),
			...
			ncols = _cols;
	// insets
	bool ltr = (parent->GetComponentOrientation() == LCO_LEFT_TO_RIGHT);
